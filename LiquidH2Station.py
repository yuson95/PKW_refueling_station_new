...
class Investment():
    ...
    def __init__(self,senario):
        ...
        self.station = LiquidH2Station(senario)
        self.f_install = 1.3
        self.rate_exchange = 0.82  # 2018 conversion of exchange rate

    def cost_cryo_tank(self):
        """Investment of Liquid Hydrogen Cryogenic Storage Tank"""
        capacity_cryo_tank = self.station.liquid_cryogenic_tank_design()['capacity_cryo_tank_design']
        cost_cryo_tank = 991.89*(capacity_cryo_tank**0.692)*self.f_install*self.rate_exchange

        return cost_cryo_tank
    ...
    def cost_pump(self):
        ...
        n_pump = self.station.pump_design()['n_pump']
        cost_pump = n_pump*425000*self.f_install * \
            self.rate_exchange if self.station.vehicle_service_pressure == 350 else n_pump * \
            700000*self.f_install*self.rate_exchange
    ...
    def cost_compressor(self):
        ...
        if self.station.vehicle_service_pressure == 700:
    ...
    def cost_cascade(self):
        ...
        if self.station.vehicle_service_pressure == 350:
    ...
    def cost_evaporator(self):
        """Investment of Evaporator"""
        cost_evaporator = self.station.station_size / \
            24*1000+15000*self.f_install*self.rate_exchange
...
def calcu_cost_spec(s):
    ...
    e_boiloff_tank = 2 * \
        station.liquid_cryogenic_tank_design()['e_boiloff_storage']
    price_h2 = 9.5  # €/kg
    cost_lost = price_h2 * \
